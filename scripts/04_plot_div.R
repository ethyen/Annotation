setwd("C:/Users/bad_m/OneDrive - Université de Fribourg/cours/master/annotation/TP/1. TE annotation classification/2. Visualizing and comparing TE annotations from EDTA")
...
#synonymous substitution rate for Brassicaceae species: 8.22 * 10^-9 substitutions per synonymous site per year
#The insertion time (T) can be calculated using the formula T = K/2r formula, where K is the
#sequence divergence, and r is the substitution rate.
...
ggplot(rep_table.m, aes(fill = fam, x = age/1000000, weight = value/1000000)) +
  geom_bar() +
  cowplot::theme_cowplot() +
  scale_fill_brewer(palette = "Paired") +
  xlab("age (Myr)") +
  ylab("Sequence (Mbp)") +
  theme(axis.text.x = element_text(vjust = 1, size = 9, hjust = 1), plot.title = element_text(hjust = 0.5))
